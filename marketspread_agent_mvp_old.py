...
def vendor_risk_reason(vendor_name: str, vendor_records: list[VendorRecord], weather_text: str) -> str:
    """Explain why a scheduled vendor may need support before the next market.

    WHY: This turns the schedule into a proactive planning tool instead of a static list.
    """
    reasons = []
    category = APPROVED_VENDORS.get(vendor_name, "Other")

    if any(record.action_needed != "Complete" for record in vendor_records):
        reasons.append("has an open reporting or payment follow-up")

    if vendor_records and any(record.is_underperforming for record in vendor_records):
        reasons.append("has recently performed below category expectations")

    if category in {"Prepared Foods", "Baked Goods"} and ("rain" in weather_text.lower() or "wind" in weather_text.lower()):
        reasons.append("may be weather-sensitive because customers may browse less in rain or wind")

    if not vendor_records:
        reasons.append("has no recent sales history in the current dataset")

    return "; ".join(reasons)


def build_at_risk_vendor_view(scheduled_vendors: list[str], records: list[VendorRecord], weather_text: str) -> str:
    """Build a short list of scheduled vendors that may need manager attention.

    WHY: For the next market, managers need to know who may need support before the day begins.
    """
    records_by_vendor: dict[str, list[VendorRecord]] = defaultdict(list)
    for record in records:
        records_by_vendor[record.vendor_name].append(record)

    risk_rows = []
    for vendor_name in scheduled_vendors:
        reason = vendor_risk_reason(vendor_name, records_by_vendor.get(vendor_name, []), weather_text)
        if reason:
            risk_rows.append(
                "<tr>"
                f"<td>{html.escape(vendor_name)}</td>"
                f"<td>{html.escape(APPROVED_VENDORS.get(vendor_name, 'Other'))}</td>"
                f"<td>{html.escape(reason)}</td>"
                "</tr>"
            )

    if not risk_rows:
        return "<p class='complete'>No at-risk vendors identified for the upcoming market.</p>"

    return (
        "<h3>At-Risk Vendors This Week</h3>"
        "<p class='note'>These are scheduled vendors who may need support before the upcoming market.</p>"
        "<div class='table-wrap'><table>"
        "<thead><tr><th>Vendor</th><th>Category</th><th>Why They Are Flagged</th></tr></thead>"
        "<tbody>"
        + "".join(risk_rows)
        + "</tbody></table></div>"
    )
...
def build_next_market_schedule_view(selected_category: str = "All", records: list[VendorRecord] | None = None) -> str:
    ...
    <p class="note">If the schedule looks out of date, regenerate it from the current vendor attendance rules.</p>
    <form method="post" action="/reset_schedule" style="margin-bottom:12px;">
      <button type="submit">Regenerate Schedule From Rules</button>
    </form>
    {build_at_risk_vendor_view(scheduled_vendors, records, weather_text)}
    {''.join(category_cards)}
    """
...
<body>
  ...
  <div class="container">
    ...
    <details class="section-card" id="schedule-section" open>
      ...
      <div class="section-body">
        {build_next_market_schedule_view(schedule_category, records)}
      </div>
    </details>
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/reset_schedule":
            self.handle_reset_schedule()
            return
    ...
    def handle_reset_schedule(self) -> None:
        """Regenerate vendor_schedule.csv from the current scheduling rules.

        WHY: The schedule CSV may have been created before the latest vendor attendance
        rules were added. This button lets the manager refresh the schedule without
        deleting files manually.
        """
        create_sample_schedule()
        self.redirect_with_message("Regenerated vendor schedule from the current attendance rules.")
